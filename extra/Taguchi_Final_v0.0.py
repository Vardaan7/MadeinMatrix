...
def run(data, d):
    ...
    if d==0:
        for j in range(t):
            for i in range(n+1):
                dataset[j][i] = int(dataset[j][i].get())
    else:
        dataset = data[1:]
    #    for i in range(n+1):
    ...
    plot_data = Toplevel(main_window)
    ...
    ### Predicting Window
    def predict_input():
        prediction_input = Toplevel(main_window)
...
def enter_data_details_window():
    ...
    def input_data_window():
        ...
        make_calculations = Button(input_data, text="Run", command=run(data=None, d=0))
...
# will be opened
def load_data():
    data_file_src = askopenfile(mode ='r', filetypes =[('CSV Files', '*.csv')])
    #print(data_file)

    #global dataset
    data_file = csv.reader(data_file_src)
    data_load = []
...
Data_Load = Button(main_window, text ='Load Data and Run', command = lambda:load_data())
Data_Load.grid(row=1, column=0, padx=90, pady=20)
